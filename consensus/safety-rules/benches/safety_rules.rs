...
fn in_memory(n: u64) {
    ...
    let safety_rules_manager = SafetyRulesManager::new_local(storage, false, false, true);
...
fn serializer(n: u64) {
    ...
    let safety_rules_manager = SafetyRulesManager::new_serializer(storage, false, false, true);
    lsr(safety_rules_manager.client(), signer, n);
}
...
fn thread(n: u64) {
    ...
    let safety_rules_manager =
        SafetyRulesManager::new_thread(storage, false, false, timeout_ms, true);
    lsr(safety_rules_manager.client(), signer, n);
}
